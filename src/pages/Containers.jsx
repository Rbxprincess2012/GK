...
// Справочник размеров контейнеров: только объём и выбор стандартного (по умолчанию
// подставляется в заявке). Физический учёт контейнеров (номер/состояние/где) не ведём —
// номер при необходимости менеджер указывает прямо в заявке.
export default function Containers() {
  ...
  const saveT = async () => {
    ...
    try {
      ...
      toast.error(saveErr(e, 'Такой размер уже есть'))
    }
  }
  const delT = async (t) => {
    if (!(await toast.confirm(`Удалить размер «${t.name}»?`))) return
    try { await removeType(t.id); toast.success('Удалено') } catch { toast.error('Нельзя удалить (используется в заявках)') }
  }
  ...
  return (
    <div className="a-page">
      <div className="a-page-header">
        <h2>Размеры контейнеров <span className="a-count">{types.length}</span></h2>
          ...
          <tbody>
            {types.map((t) => (
              <tr key={t.id}>
                ...
                <td className="a-muted">{t.volume != null ? `${Number(t.volume)} м³` : '—'}</td>
                <td>
            ...
            {types.length === 0 && <tr><td colSpan={4} className="a-loading">Размеров нет — добавьте, например «8 м³»</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {tEdit && (
        ...
        >
          <label className="a-field"><span>Название</span>
            <input className="a-input" value={tForm.name} onChange={(e) => setTForm({ ...tForm, name: e.target.value })} placeholder="напр. Лодочка 8 м³" />
          </label>

...
export function createMaxClientBot(token) {
  ...
  // Бота добавили в чат. В MAX рядовой участник НЕ видит сообщения других — поэтому, пока бот
  // не администратор, команда /bind до него не дойдёт. Подсказываем это сразу (best-effort:
  // если прав на запись ещё нет — молча игнорируем, сработает после назначения админом).
  bot.on('bot_added', async (ctx) => {
    await ctx.reply([
      'Здравствуйте! Я бот уведомлений о выполнении заявок.',
      '',
      'Чтобы привязать эту группу и получать сюда отчёты:',
      '1) Назначьте меня АДМИНИСТРАТОРОМ чата — без этого MAX не даёт мне видеть сообщения.',
      '2) Отправьте сюда команду: /bind <код от менеджера>.',
      '',
      'После этого я отвечу «✅ Привязано».',
    ].join('\n')).catch(() => {})
  })
